...
void DataSourceHearslam::startStreaming() {
  streaming_thread_.reset(
      new std::thread(std::bind(&DataSourceHearslam::streamingWorker, this)));
}
...
void DataSourceHearslam::initialize() {
  try {
  ...
  try {
    CHECK(bag_);
    if (FLAGS_hearslam_data_end_s != 0.0 ||
        FLAGS_hearslam_data_start_s != 0.0) {
      ...
      CHECK_GE(FLAGS_hearslam_data_start_s, 0);
      const double absolute_start_time_s =
          bag_start_time + FLAGS_hearslam_data_start_s;

      // Auto-set the end-time to the bag-length if not provided.
      double hearslam_data_end_s = FLAGS_hearslam_data_end_s;
      if (hearslam_data_end_s <= 0.0) {
        hearslam_data_end_s = bag_view_->getEndTime().toSec() - bag_start_time;
      }
      CHECK_GT(hearslam_data_end_s, FLAGS_hearslam_data_start_s);
      const double absolute_end_time_s = bag_start_time + hearslam_data_end_s;
...
void DataSourceHearslam::streamingWorker() {
  // Play all messages.
  CHECK(bag_view_);
  CHECK_GT(FLAGS_hearslam_data_realtime_playback_rate, 0.0);
  const double message_wait_time_scaler =
      1.0 / FLAGS_hearslam_data_realtime_playback_rate;
